...
from curve_apps.peak_finder.layout import object_selection_layout
...
class ObjectSelection:
    ...
    def update_object_options(
        ...
        """
        This function is called when a file is uploaded. It sets the new workspace,
        sets the dcc ui_json_data component, and sets the new object options and
        values.
        ...
        :return filename: Return None to reset the filename so the same file can
            be chosen twice in a row.
        :return contents: Return None to reset the contents so the same file can
            be chosen twice in a row.
        """
        if trigger is None:
        ...
        if contents is not None and filename is not None:
            if filename.endswith(".ui.json"):
                ...
                # Update self.params with new workspace, but keep unaffected
                # params the same.
    ...
    @app_name.setter
    def app_name(self, val: str | None):
    ...
    def workspace(self, val):
        if not isinstance(val, Workspace | None):
            raise TypeError(
                "Value for attribute `workspace` should be "
                ":obj:`geoh5py.workspace.Workspace` or 'None'"
            )
        if self._workspace is not None:
            # Close old workspace
            self._workspace.close()
