...
def test_version_is_consistent():
    project_version = Version(curve_apps.__version__)
...
def _version_module_exists():
    try:
        importlib.import_module("curve_apps._version")
        return True
...
@pytest.mark.skipif(
    ...
    reason="curve_apps._version can be found: package is built",
)
...
@pytest.mark.skipif(
    ...
    reason="curve_apps._version cannot be found: uses a fallback version",
)
...
def test_version_is_valid():
    assert validate_version(curve_apps.__version__) is not None
